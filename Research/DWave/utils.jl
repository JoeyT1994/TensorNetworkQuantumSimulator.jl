...
#Assume symmetry
function zz_correlations_bp(ψ::ITensorNetwork, v, vs = collect(vertices(ψ)); bp_update_kwargs = (; maxiter =50, tol = 1e-14, message_update_kwargs = (; message_update_function = ms -> make_eigs_real.(default_message_update(ms)))))
    ψIψ = BeliefPropagationCache(QuadraticFormNetwork(ψ))
    s = only(inds(only(factors(ψIψ, [(v, "operator")])); plev = 0))
    pg = partitioned_tensornetwork(ψIψ)
    pg = rem_vertex(pg, (v, "operator"))
    ψIψ = BeliefPropagationCache(pg, messages(ψIψ))
    ψIψ = update_factor(ψIψ, (v, "ket"), ψ[v]*onehot(s => 1))
    ψIψ = update_factor(ψIψ, (v, "bra"), dag(prime(ψ[v]))*onehot(s' => 1))
    println("Updating")
    ...
    corrs = Dictionary()
    for vn in vs
        if findfirst(x -> x == vn, vs) > findfirst(x -> x == v, vs)
            ρ = contract(environment(ψIψ, [(vn, "operator")]); sequence = "automatic")
            p_upup, p_downup = diag(ρ)[1], diag(ρ)[2]
            szsz = (p_upup - p_downup) / (p_upup + p_downup)
            set!(corrs, NamedEdge(v =>vn), szsz)
        end
    end
    return corrs
end
...
#Assume symmetry
function zz_correlations_bp_loopcorrect(ψ::ITensorNetwork, v, max_loop_length::Int64; bp_update_kwargs = (; maxiter =50, tol = 1e-14, message_update_kwargs = (; message_update_function = ms -> make_eigs_real.(default_message_update(ms)))))
    ...
    vns = filter(vn -> findfirst(x -> x == vn, vs) > findfirst(x -> x == v, vs), vs)
    ρs = loop_corrected_one_site_rdms(ψIψ, vns, max_loop_length)
    corrs = Dictionary()
...
function insert_szsz(ψ::ITensorNetwork, ψIψ::BeliefPropagationCache, v1, v2)
    ψOψ, ψ = copy(ψIψ), copy(ψ)
    s1 = only(inds(only(factors(ψIψ, [(v1, "operator")])); plev = 0))
    s2 = only(inds(only(factors(ψIψ, [(v2, "operator")])); plev = 0))

    ψOψ = update_factor(ψOψ, (v1, "operator"), ITensors.op("Z", s1))
    ψOψ = update_factor(ψOψ, (v2, "operator"), ITensors.op("Z", s2))
    return ψOψ, ψ
end 
...
function project(ψ::ITensorNetwork, ψIψ::BeliefPropagationCache, v1, v2, v1_val::Int64 = 1, v2_val::Int64=1)
    ...
    pg = partitioned_tensornetwork(ψIψ)
    pg = rem_vertex(pg, (v1, "operator"))
    pg = rem_vertex(pg, (v2, "operator"))
    ψIψ = BeliefPropagationCache(pg, messages(ψIψ))
    ψIψ = update_factor(ψIψ, (v1, "ket"), ψ[v1]*onehot(s1 => v1_val))
    ψIψ = update_factor(ψIψ, (v1, "bra"), dag(prime(ψ[v1]))*onehot(s1' => v1_val))
    ψIψ = update_factor(ψIψ, (v2, "ket"), ψ[v2]*onehot(s2 => v2_val))
    ψIψ = update_factor(ψIψ, (v2, "bra"), dag(prime(ψ[v2]))*onehot(s2' => v2_val))
    ψ[v1] = ψ[v1]*onehot(s1 => v1_val)
    ψ[v2] = ψ[v2]*onehot(s2 => v2_val)
    return ψIψ, ψ
end 
