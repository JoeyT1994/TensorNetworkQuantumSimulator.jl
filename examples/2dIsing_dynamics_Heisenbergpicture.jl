...
function main()
    ...
    append!(layer, [op("Rz", s[v][1];  θ = -h * δt)*op("Rz", s[v][2];  θ = h * δt) for v in vertices(g)])
    for es in ec
        append!(layer, [op("Rxx", s[src(e)][1], s[dst(e)][1], ϕ = -J * δt)*op("Rxx", s[src(e)][2], s[dst(e)][2], ϕ = J * δt) for e in es])
    end
    append!(layer, [op("Rz", s[v][1];  θ = -h * δt)*op("Rz", s[v][2];  θ = h * δt) for v in vertices(g)])
