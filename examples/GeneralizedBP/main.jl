...
function main()
    ...
    for i in 1:nsamples
        ...
        gbp_f, msgs, gbp_converged = generalized_belief_propagation(ψ_bpc, bs, ms, ps, cs, b_nos, mobius_nos; niters = 750, rate = 0.3)
        ...
        for e in es
            ...
            b_gbp = b_beta(beta, get_psi(ψ_bpc, ms[beta]), msgs, ps, b_nos; normalize = false)
        ...
        for v in collect(vertices(g))
            ...
            b_gbp = b_alpha(alpha, get_psi(ψ_bpc, bs[alpha]), msgs, cs, ps; normalize = false)
    ...
    println("-------------------------------------")
    println("Average simple BP absolute error on free energy over $nconverged_samples converged samples: ", err_bp / nsamples)
    println("Average generalized BP absolute error on free energy over $nconverged_samples converged samples: ", err_gbp / nsamples)
    println("Average loop corrected BP absolute error on free energy over $nconverged_samples converged samples: ", err_lc / nsamples)

    println("Average simple BP absolute error on marginals over $nconverged_samples converged samples: ", err_bp_marginals/nsamples)
    println("Average GBP absolute error on marginals over $nconverged_samples converged samples: ", err_gbp_marginals/nsamples)

    println("Average simple BP absolute error on vertex marginals over $nconverged_samples converged samples: ", err_bp_vertex_marginals/nsamples)
    println("Average GBP absolute error on vertex marginals over $nconverged_samples converged samples: ", err_gbp_vertex_marginals/nsamples)
end
