...
function update_message(T::BeliefPropagationCache, alpha, beta, msgs, b_nos, ps, cs, ms, bs; rate = 1.0, normalize::Bool = true)
    ...
    for _beta in cs[alpha]
        for parent_alpha in ps[_beta]
            if parent_alpha != alpha
                psi_alpha = special_multiply(psi_alpha, msgs[(parent_alpha, _beta)])		
            end
        end
    end
...
function generalized_belief_propagation(T::BeliefPropagationCache, bs, ms, ps, cs, b_nos, mobius_nos; niters::Int, rate::Number, tol=1e-12, verbose = false, prev_msgs = nothing)
    ...
    for i in 1:niters
        ...
        if verbose
            println("GBP iteration $i complete. Average difference in messages: $(diffs[i])")
        end
        if i % niters == 0 && verbose
...
function b_beta(beta, psi_beta, msgs, ps, b_nos)
    b = copy(psi_beta)
    for alpha in ps[beta]
        n = elementwise_operation(x -> iszero(x) ? 0.0 : x^(b_nos[beta]), msgs[(alpha, beta)])
