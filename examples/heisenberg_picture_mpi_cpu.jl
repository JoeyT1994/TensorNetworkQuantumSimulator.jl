...
using ITensors
using ITensors:
    @OpName_str, @SiteType_str, ITensor, Index, delta, denseblocks, op, replaceind
using LinearAlgebra
using NamedGraphs
...
function run_circuit_mpi(maxdim::Int, L::Int, sc::Float64; delta = 0.3)
    ...
    η = (1 - sc) * 0.125
    dir =  "/mnt/home/jtindall/ceph/Data/Algorthmic/Circuits/56q_circuits/"
    ...
    apply_kwargs = (; maxdim = maxdim, cutoff = 1.0e-8, normalize_tensors = false)
    ...
    bp_update_kwargs = (; maxiter = 25, tolerance = 1.0e-5)
    num = real(TN.inner_mpi(O, M, super_graph, shared_vertices; comm, bp_update_kwargs))
    S = num / den
    # errs only covers this rank's gates; log space avoids underflow in the global product
    fid = exp(MPI.Allreduce(sum(log1p.(-errs)), MPI.SUM, comm))
    rank == root && println(
        ">>> QUAD η=$η χ=$maxdim : 
...
Ls = [parse(Int64, ARGS[1])]
scs = [parse(Float64, ARGS[2])]
chi = 1024
println("Chi is $chi")
for L in Ls
    println("L is $L")
