...
extern "C" int compute_periodic_schur_eigenvalues_D(
    const double* T, int period, int input_width, int n, void* eigenvalues);

extern "C" int compute_periodic_schur_eigenvalues_Z(
    const void* T, int period, int input_width, int n, void* eigenvalues);

extern "C" int compute_periodic_schur_active_D(
    const double* H, const unsigned char* active_cols,
    double schur_deflation_tol, int period, int m, int n, int output_width,
...
// Read all eigenvalues of static real periodic Schur factors.
ffi::Error PeriodicSchurEigenvaluesRealImpl(
    F64R3 T,
    ffi::ResultBuffer<ffi::C128, 1> eigenvalues) {
  const int period = static_cast<int>(T.dimensions()[0]);
  const int n = static_cast<int>(T.dimensions()[1]);
  if (period < 1 || T.dimensions()[2] != n) {
    return ffi::Error::InvalidArgument(
        "T must have shape (period,n,n) with period >= 1");
  }
  const int info = compute_periodic_schur_eigenvalues_D(
      T.typed_data(), period, n, n, eigenvalues->typed_data());
  if (info != 0) {
    return ffi::Error::Internal(
        "real periodic Schur eigenvalue readout failed with info=" +
        std::to_string(info));
  }
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(
    PeriodicSchurEigenvaluesRealF64,
    PeriodicSchurEigenvaluesRealImpl,
    ffi::Ffi::Bind()
        .Arg<F64R3>()
        .Ret<C128R1>());

// Read all eigenvalues of static complex periodic Schur factors.
ffi::Error PeriodicSchurEigenvaluesComplexImpl(
    C128R3 T,
    ffi::ResultBuffer<ffi::C128, 1> eigenvalues) {
  const int period = static_cast<int>(T.dimensions()[0]);
  const int n = static_cast<int>(T.dimensions()[1]);
  ...
  const int info = compute_periodic_schur_eigenvalues_Z(
      T.typed_data(), period, n, n, eigenvalues->typed_data());
  if (info != 0) {
    return ffi::Error::Internal(
        "complex periodic Schur eigenvalue readout failed with info=" +
        std::to_string(info));
  }
  return ffi::Error::Success();
}
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(
    PeriodicSchurEigenvaluesComplexC128,
    PeriodicSchurEigenvaluesComplexImpl,
    ffi::Ffi::Bind()
        .Arg<C128R3>()
        .Ret<C128R1>());
...
ffi::Error PeriodicSchurActiveRealImpl(
  ...
  if (H.dimensions()[2] != m ||
      ...
      schur_deflation_tol.typed_data()[0] < 1.0) {
    return ffi::Error::InvalidArgument(
        "H and active_cols must have shapes (period,m,m) and (period,m), "
        "and schur_deflation_tol must be at least 1");
  }
  const auto* active =
  ...
  const int info = compute_periodic_schur_active_D(
      H.typed_data(), active, schur_deflation_tol.typed_data()[0],
      period, m, n, m,
