...
# correlators. The hopping evolution conserves particle number dynamically, while fZ2
# enforces only total parity structurally.
#
...
#      hopping ⟨c†c⟩ and on-site density ⟨N⟩ against the exact values.
...
function evolve(g, occupied; tt = 1.0, dt = 0.05, nsteps = 10, χ = 16)
    ...
    for _ in 1:nsteps
        ψ_bpc, _ = apply_gates(layer, ψ_bpc; apply_kwargs = (; maxdim = χ, cutoff = 1.0e-16))
    end
...
function main(; tt = 1.0, dt = 0.0025, nsteps = 200)
    ...
    println("\n== 2. 3×3 CDW quench, boundary-MPS observable checks ==")
    g = named_grid((3, 3))
    occupied = filter(v -> isodd(sum(v)), collect(vertices(g)))
    ψ, T = evolve(g, occupied; tt, dt, nsteps)
    C, idx = correlation_matrix(g, tt, T, occupied)
