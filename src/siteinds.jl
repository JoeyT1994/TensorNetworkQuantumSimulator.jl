...
function siteinds(sitetype::String, g::AbstractGraph, sitedimension::Integer = site_dimension(sitetype); inds_per_site::Integer = 1, sectors = nothing, symmetry::String = "U1")
    ...
    if replace(lowercase(sitetype), " " => "") ∈ ["spinfulfermion", "electron"]
        #spinful fermionic sites, charge-ordered basis (|0⟩, |↑↓⟩ | |↑⟩, |↓⟩)
        return Dictionary(vs, [[KTensors.new_fermion_index(2, 2; tags = site_tag(sitetype)) for i in 1:inds_per_site] for v in vs])
    end
